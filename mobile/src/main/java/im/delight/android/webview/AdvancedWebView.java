...
public class AdvancedWebView extends WebView {
	...
	@SuppressWarnings("all")
	public void onPause() {
		// this is commented out to fix a bug with infinite loading of a page run from navigation menu, this bug occurred after updating support library to 25.1.0
		// pauseTimers();
		if (Build.VERSION.SDK_INT >= 11) {
			super.onPause();
		}
		...
		super.setWebChromeClient(new WebChromeClient() {
			...
			public void onGeolocationPermissionsShowPrompt(String origin, Callback callback) {
				if (mGeolocationEnabled) {
					...
					mListener.onGeolocationPermissionsShowPrompt();
				}
				else {
					if (mCustomWebChromeClient != null) {
